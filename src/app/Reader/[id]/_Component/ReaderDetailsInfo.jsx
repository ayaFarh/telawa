...
export default function ReaderDetailsInfo() {
  ...
  const [likedSurahs, setLikedSurahs] = useState([]); // ✅ جديد
  ...
  onClick={async (e) => {
    e.stopPropagation();
    try {
      const response = await fetch(`${selectedMoshaf?.server}${padded}.mp3`);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);

      const link = document.createElement("a");
      link.href = url;
      link.download = `${padded}.mp3`;
